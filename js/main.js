function getGithubData(){
  ...
  var feed = new Instafeed({
    ...
    template: '<li><a href="{{link}}"><img src="{{image}}" /></a><p class="name">{{caption}}</p><p><3 {{likes}}</p><abbr class="date instafeed">{{model.created_time}}</abbr></li>',
    after: function() {
  ...
  feed.run();
  $(document).ajaxComplete(function(){
    $('abbr').each(function(){ this.setAttribute('title', new Date(parseInt((this.innerHTML).toString() + '000')).toISOString()); });
    $('abbr').timeago();
  });
